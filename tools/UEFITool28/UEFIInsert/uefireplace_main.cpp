...
int main(int argc, char *argv[])
{
    ...
    a.setApplicationName("UEFIInsert");
    ...
    if (args.length() < 5) {
        std::cout << "UEFIInsert " PROGRAM_VERSION " - UEFI image file insertion utility" << std::endl << std::endl <<
            "Usage: UEFIInsert image_file guid section_type contents_file [-o output] [-before]" << std::endl;
        return ERR_SUCCESS;
    }
    ...
    if (!converted) {
        ...
        if (result == ERR_SUCCESS) {
            result = uefitool.insert(args.at(1), guid, sectionType, args.at(4), output, after);
        }
    }

    switch (result) {
    case ERR_SUCCESS:
        std::cout << "File inserted" << std::endl;
        break;
